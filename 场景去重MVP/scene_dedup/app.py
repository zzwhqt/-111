...
from .text_rerank import router as text_rerank_router
from .visual_eval import router as visual_eval_router
...
app.include_router(text_rerank_router)
app.include_router(visual_eval_router)
...
def _run_search(
    ...
    try:
        ...
        hits = [
            ...
            for item in store().search(task_vector, frame_vectors, top_k, query_text=None)
        ]
        return {
            "index_size": len(store().metadata),
            "score_semantics": "0-100 pure-visual ranking score (60% task cosine + 40% frame coverage); it is not a calibrated duplicate probability",
            "query_mode": "pure_visual_scene_v1",
...
<div class="eyebrow">CPU-only · CLIP + Faiss</div><h1>Task 场景重复检索</h1><p>输入 OSS 视频地址与 Task 帧范围，系统只使用该片段的8帧视觉特征，在1000条资产中返回Top-K相似场景；不会读取或使用文字描述。帧区间采用 <code>[start_frame, end_frame)</code>，相似度是库内归一化检索分，不等同于重复概率。 <a href="/label" style="color:#5eead4">进入2000条Task人工盲标工作台 →</a></p>
...
form.addEventListener('submit',async e=>{e.preventDefault();const payload={url:document.getElementById('videoUrl').value.trim(),start_frame:Number(document.getElementById('startFrame').value),end_frame:Number(document.getElementById('endFrame').value),fps:Number(document.getElementById('fps').value),top_k:Number(document.getElementById('topK').value)};button.disabled=true;statusEl.className='status';statusEl.textContent='正在读取片段、抽帧、编码并匹配…';results.replaceChildren(node('div','panel empty','检索处理中，CPU 首次加载模型可能需要几十秒'));try{const response=await fetch('/search/url',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),data=await response.json();if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);statusEl.textContent=`完成 · ${data.query_mode}`;render(data.results||[],data.index_size)}catch(err){statusEl.className='status error';statusEl.textContent='检索失败';results.replaceChildren(node('div','panel empty error',String(err.message||err)))}finally{button.disabled=false}});
</script></body></html>"""
