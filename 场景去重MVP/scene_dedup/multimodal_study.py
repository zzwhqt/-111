...
def _scene_asset(task_id: str, metadata: dict[str, Any]) -> dict[str, Any]:
    # Scene round deliberately omits scene/task_name/details and model scores.
    return {
        "task_id": task_id,
...
<main><div class="intro"><div><h1>只判断工作场景，不判断人在做什么</h1><div class="hint">本轮隐藏 Task 描述和模型分数。请依次查看左侧查询视频与右侧5个资产视频，然后给每一对选择一个场景等级。点击画面只负责播放，不会选择标签。</div></div><div class="legend"><span>3 几乎同一工位</span><span>2 工位布局明显相似</span><span>1 同类环境</span><span>0 环境不同</span><span>9 无法判断</span></div></div><div id="workspace"></div><div class="toolbar"><button class="primary" id="submit" disabled>提交这5对并进入下一条</button><button class="secondary" id="undo">撤销上一组</button><span class="status" id="status"></span></div></main>
<script>
